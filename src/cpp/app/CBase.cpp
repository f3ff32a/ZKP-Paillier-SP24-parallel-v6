...
namespace
{
void releaseMatrixVectorDeep(vector<shared_ptr<Matrix>> &matrices)
{
  for (auto &matrix : matrices)
  {
    if (matrix != nullptr)
      matrix->releaseMemory();
  }
  releaseStdVector(matrices);
}
} // namespace
...
void CBase::releaseMemory()
{
  releaseMatrixVectorDeep(Wqa);
  releaseMatrixVectorDeep(Wqb);
  releaseMatrixVectorDeep(Wqc);
  releaseNtlVec(Kq);
...
shared_ptr<CircuitZKPVerifier> CBase::generateVerifierAndRelease(const Vec<ZZ_p> &gi)
{
  auto mnCfg = CircuitZKPVerifier::calcMN(gateCount);
  auto m = mnCfg[0];
  auto n = mnCfg[1];
  ...
  convertWire(this->Wqa, zkp->Wqa, m, n);
  releaseMatrixVectorDeep(this->Wqa);
  trimProcessMemory();

  convertWire(this->Wqb, zkp->Wqb, m, n);
  releaseMatrixVectorDeep(this->Wqb);
  trimProcessMemory();

  convertWire(this->Wqc, zkp->Wqc, m, n);
  releaseMatrixVectorDeep(this->Wqc);
  releaseNtlVec(this->Kq);
  trimProcessMemory();
...
shared_ptr<CircuitZKPProver> CBase::generateProverAndRelease(const Vec<ZZ_p> &gi)
{
  auto zkp = generateVerifierAndRelease(gi);

  auto groupedA = A->group(zkp->n, zkp->m);
  A->releaseMemory();
  A.reset();
  trimProcessMemory();

  auto groupedB = B->group(zkp->n, zkp->m);
  B->releaseMemory();
  B.reset();
  trimProcessMemory();

  auto groupedC = C->group(zkp->n, zkp->m);
  C->releaseMemory();
  C.reset();
  trimProcessMemory();

  auto prover = make_shared<CircuitZKPProver>(zkp, groupedA, groupedB, groupedC);

  groupedA->releaseMemory();
  groupedB->releaseMemory();
  groupedC->releaseMemory();
  groupedA.reset();
  groupedB.reset();
  groupedC.reset();
  trimProcessMemory();
