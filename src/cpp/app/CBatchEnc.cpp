...
// Convert NTL::ZZ to decimal string outside the OpenMP region.
// This keeps NTL operations away from worker threads.
static string zzToDecimalString(const ZZ &x)
{
  ostringstream oss;
  oss << x;
  return oss.str();
...
static ZZ decimalStringToZZ(const string &s)
{
  ZZ x;
  istringstream iss(s);
  iss >> x;
...
// Thread-safe Paillier encryption implemented with GMP only.
// c = (1 + n*m) * r^n mod n^2.
// All mpz_t variables are local to the calling thread.
static string paillierEncryptGMP(const string &nDec,
                                const string &n2Dec,
                                const string &mDec,
                                const string &rDec)
{
  mpz_t n, n2, m, r, left, right, c;
  mpz_inits(n, n2, m, r, left, right, c, NULL);

  mpz_set_str(n, nDec.c_str(), 10);
  mpz_set_str(n2, n2Dec.c_str(), 10);
  mpz_set_str(m, mDec.c_str(), 10);
  mpz_set_str(r, rDec.c_str(), 10);

  // left = (n * m + 1) mod n2
  mpz_mul(left, n, m);
  mpz_add_ui(left, left, 1);
  mpz_mod(left, left, n2);

  // right = r^n mod n2
  mpz_mod(r, r, n2);
  mpz_powm(right, r, n, n2);

  // c = left * right mod n2
  mpz_mul(c, left, right);
  mpz_mod(c, c, n2);

  char *raw = mpz_get_str(NULL, 10, c);
  string result(raw);
  free(raw);

  mpz_clears(n, n2, m, r, left, right, c, NULL);
  return result;
}
...
#ifdef _OPENMP
  cout << "[CBatchEnc::encrypt] OpenMP ENABLED, _OPENMP = " << _OPENMP << endl;
  cout << "[CBatchEnc::encrypt] max threads = " << omp_get_max_threads() << endl;
#else
  cout << "[CBatchEnc::encrypt] OpenMP NOT ENABLED" << endl;
#endif

  // Generate randomness serially.  NTL random state and ZZ_p modulus contexts
  // are deliberately kept outside the OpenMP region.
  Rm.SetLength(msgCount);
  Cm.SetLength(msgCount);

  // Prepare all values used by worker threads as decimal strings.
  // The OpenMP region below must not call NTL arithmetic, because many
  // libntl packages are not built with thread-local arithmetic contexts.
  string nDec = zzToDecimalString(crypto->getPublicKey());
  string n2Dec = zzToDecimalString(crypto->getGroupP());
  vector<string> mDec(msgCount);
  vector<string> RmDec(msgCount);
  vector<string> CmDec(msgCount);

  for (size_t i = 0; i < msgCount; i++)
  {
    Rm[i] = crypto->pickRandom();
    mDec[i] = zzToDecimalString(m[i]);
    RmDec[i] = zzToDecimalString(conv<ZZ>(Rm[i]));
  }

  // Parallelize only the independent normal-message encryptions C_m.
  // GMP is used inside worker threads to avoid NTL thread-safety issues.
  // Other auxiliary values are left in the original serial order to keep
  // the witness/constraint assignment exactly consistent with the protocol.
#pragma omp parallel
  {
#pragma omp single
    {
#ifdef _OPENMP
      cout << "[CBatchEnc::encrypt] actual threads = " << omp_get_num_threads() << endl;
#endif
    }

#pragma omp for schedule(dynamic)
    for (long ii = 0; ii < (long)msgCount; ii++)
    {
      size_t i = (size_t)ii;
      CmDec[i] = paillierEncryptGMP(nDec, n2Dec, mDec[i], RmDec[i]);
    }
  }

  {
    ZZ_pPush push(GP_P);
    for (size_t i = 0; i < msgCount; i++)
      Cm[i] = conv<ZZ_p>(decimalStringToZZ(CmDec[i]));
  }
