...
void polyu::end_to_end(const shared_ptr<PaillierEncryption> &crypto, size_t msgCount, size_t rangeProofCount, size_t slotSize, size_t msgPerBatch, ofstream &fs)
{
  ...
  auto prover = proverCir->generateProverAndRelease(gi);
  ...
  // Rm is not used after this point; avoid copying it to reduce peak memory.
  ...
  proverCir->releaseMemory();
  proverCir.reset(); // clean up, save memory
  trimProcessMemory();
  ...
  prover->releaseMemory();
  prover.reset(); // clean up, save memory
  trimProcessMemory();
  ...
  auto verifier = verifierCir->generateVerifierAndRelease(gi);
  verifierCir->releaseMemory();
  verifierCir.reset(); // clean up, save memory
  ...
  const auto matrixM = verifier->m;
  const auto matrixN = verifier->n;
  const auto ljLen = Lj.length();
  const auto commitsLen = commits.length();
  const auto pcLen = pc.length();
  const auto proofsLen = proofs.length();
  verifier->releaseMemory();
  verifier.reset();

  // After verification, these proof/circuit-transfer values are no longer
  // needed by the end-to-end aggregation phase.  Release them before the
  // next phase starts to lower peak memory.
  releaseNtlVec(Cm_);
  releaseNtlVec(CRj);
  releaseNtlVec(Lj);
  releaseNtlVec(gi);
  releaseNtlVec(commits);
  releaseNtlVec(pc);
  releaseNtlVec(proofs);
  releaseStdVector(ljir1);
  releaseStdVector(ljir2);
  trimProcessMemory();
...
// }
  ...
  const auto m = matrixM;
  const auto n = matrixN;
  ...
  proofSize += pSize * ljLen;
  proofSize += qSize * commitsLen;
  proofSize += qSize * pcLen;
  proofSize += pSize * proofsLen;
  ...
  releaseStdVector(ljir1);
  releaseStdVector(ljir2);
  releaseStdVector(ave_result1);
  releaseStdVector(ave_result2);

  if (encCir != nullptr)
  {
    encCir->releaseMemory();
    encCir.reset();
  }
  decryptor.reset();
  encryptor.reset();
