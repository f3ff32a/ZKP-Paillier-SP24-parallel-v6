...
public:
  ...
  /**
   * @brief Explicitly release large circuit matrices and constraint vectors.
   *
   * Use this after converting the circuit into a prover/verifier object, or at
   * the end of a benchmark run, to avoid keeping old circuit data alive before
   * the next large parameter set starts.
   */
  ...
  /**
   * @brief Generate CircuitZKPVerifier and release this circuit's wire matrices during conversion.
   *
   * This is intended for large benchmark runs where the source CBase object
   * will not be used again after creating the verifier/prover object.
   */
  shared_ptr<CircuitZKPVerifier> generateVerifierAndRelease(const Vec<ZZ_p> &gi);
  ...
  /**
   * @brief Generate CircuitZKPProver and release this circuit's wire/value matrices during conversion.
   *
   * This lowers the peak memory usage of P.circuit because the old sparse
   * circuit representation is discarded as soon as it has been converted.
   */
  shared_ptr<CircuitZKPProver> generateProverAndRelease(const Vec<ZZ_p> &gi);
