...
{
  releaseNtlVec(this->cachedY_Mq);
  releaseNtlVec(this->cachedY);
  releaseNtlVec(this->cachedY_);
  getY_Mq(y);
  getY(y);
  getY_(y);
}
...
void CircuitZKPVerifier::releaseMemory()
{
  releaseNtlVec(cachedY);
  releaseNtlVec(cachedY_);
  releaseNtlVec(cachedY_Mq);

  decltype(Wqa) emptyWqa;
  Wqa.swap(emptyWqa);
  decltype(Wqb) emptyWqb;
  Wqb.swap(emptyWqb);
  decltype(Wqc) emptyWqc;
  Wqc.swap(emptyWqc);

  releaseNtlVec(Kq);
  releaseNtlVec(commitA);
  releaseNtlVec(commitB);
  releaseNtlVec(commitC);
  releaseNtlVec(pc);
  clear(commitD);
